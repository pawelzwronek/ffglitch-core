...
install-lib$(NAME)-shared: $(SUBDIR)$(SLIBNAME)
	...
	$(if $(SLIB_INSTALL_EXTRA_SHLIB),$(if $(wildcard $(SLIB_INSTALL_EXTRA_SHLIB:%=$(SUBDIR)%)),$$(INSTALL) -m 644 $(SLIB_INSTALL_EXTRA_SHLIB:%=$(SUBDIR)%) "$(SHLIBDIR)"))
	$(if $(SLIB_INSTALL_EXTRA_LIB),$(Q)mkdir -p "$(LIBDIR)")
	$(if $(SLIB_INSTALL_EXTRA_LIB),$(if $(wildcard $(SLIB_INSTALL_EXTRA_LIB:%=$(SUBDIR)%)),$$(INSTALL) -m 644 $(SLIB_INSTALL_EXTRA_LIB:%=$(SUBDIR)%) "$(LIBDIR)"))
